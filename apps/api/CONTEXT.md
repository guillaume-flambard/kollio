...
**Critic**:
The component that proposes Challenge findings automatically: unsupported assumptions, contradictory evidence, hidden dependencies, failure modes, causal claims and missing success criteria. Opening a challenge dispatches a run on the `challenge.execute` worker task, which builds a brief from the Option and its confirmed evidence, makes one call on the visible tier, checks every returned finding against the six kinds and the citations that were sent, and stores them `proposed` with origin `critic`. The model that answered is recorded on the run. Checking happens before any write, so a call that fails, a result that is partly unusable and a result that proposes nothing each end the run `FAILED` with a stored reason and no findings. The API still never lets a client declare a `critic` origin, and nothing about the Critic ranks or scores an Option.
