...
def test_member_can_read_scenarios():
    assert can_read_scenarios(WORKSPACE, frozenset({WORKSPACE})) is True


def test_membership_in_another_workspace_does_not_grant_a_read():
    assert can_read_scenarios(WORKSPACE, frozenset({OTHER_WORKSPACE})) is False


def test_non_member_cannot_read_scenarios():
    assert can_read_scenarios(WORKSPACE, frozenset()) is False


def test_owner_can_write():
    assert can_write_scenarios(OWNER, frozenset({OWNER}), OWNER) is True


def test_participant_can_write():
    assert can_write_scenarios(OWNER, frozenset({OWNER, PARTICIPANT}), PARTICIPANT) is True


def test_uninvolved_member_cannot_write():
    assert can_write_scenarios(OWNER, frozenset({OWNER}), MEMBER) is False
