...
async def _seed(session, count=3):
    ...
    for index in range(count):
        ...
        latest.append(Iteration(
            ...
            lang="en",
            payload={"title": f"Explorer idea {index}", "pitch": "A coop fleet", "stage": "seed"},
            branch="main",
            proposal_status=None,
            short_hash=iteration_id.hex[:12],
            revision=1,
            created_at=datetime.now(timezone.utc) - timedelta(hours=(30 - index)),
        ))
        latest.append(Iteration(
            id=extra,
            idea_id=idea_id,
            ...
            revision=2,
            created_at=datetime.now(timezone.utc) - timedelta(minutes=(50 - index * 5)),
        ))
    ...
    async with engine.connect() as connection:
        ...
        async with local() as session:
            ...
            await store_raw_analysis(
                repository, ideas[0].id, latest[2].id,
                _analysis_payload(realism_score=75), model="test",
            )
            await store_raw_analysis(
                repository, ideas[2].id, latest[-1].id,
                _analysis_payload(realism_score=40), model="test",
            )
            app = create_app(Settings(_env_file=None, database_url=url))
            async with _client(session, app, url, owner_id) as client:
                by_role = await client.get(
                    f"/workspaces/{workspace_id}/ideas?sought_role=dev"
                )
                assert by_role.status_code == 200
                assert {tuple(item["sought_roles"]) for item in by_role.json()["items"]} == {("dev",)}

                grounded = await client.get(
                    f"/workspaces/{workspace_id}/ideas?realism_min=60"
                )
                ...
                assert [item["realism_score"] for item in everything.json()["items"]] == [75, None, 40]
        await transaction.rollback()
...
async def test_list_still_filters_by_stage(explorer_database):
    ...
    async with engine.connect() as connection:
        ...
        async with local() as session:
            ...
            app = create_app(Settings(_env_file=None, database_url=url))
            async with _client(session, app, url, owner_id) as client:
                iterating = await client.get(
                    f"/workspaces/{workspace_id}/ideas?stage=iterating"
                )
