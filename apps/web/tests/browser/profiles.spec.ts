...
const profile: ProfileResponse = {
  ...
  memberships: [{ idea_id: 'idea-two', idea_title: 'Deuxième idée', role: 'finance' }],
...
const idea: IdeaResponse = {
  ...
  collaborators: [
    { id: 'owner-one', display_name: 'Propriétaire', participation: 'owner', business_function: 'direction', roles: ['platform'], bio: null, avatar_key: 'lilac' },
  ],
...
for (const [locale, messages] of [['fr', fr], ['en', en]] as const) {
  test.describe(locale, () => {
    ...
    const roleLabels = messages.ideas.detail.roles as Record<string, string>
    const functions = messages.ideas.function as Record<string, string>
    ...
    test('PROFILE-01 opens a profile from the idea team panel', async ({ page }) => {
      ...
      // Role and function labels resolve through the families that own their vocabulary, never as key paths.
      await expect(page.locator('.profile-roles li')).toHaveText([roleLabels.engineering, roleLabels.platform])
      await expect(page.locator('.profile-roles')).not.toContainText('ideas.')
      await expect(page.locator('.profile-section').nth(1).locator('strong')).toHaveText(functions.finance)
    ...
    test('PROFILE-02 opens a profile from an explorer collaborator', async ({ page }) => {
      ...
      await expect(page.locator('.explorer-row-topics')).toContainText(roleLabels.platform)
      await expect(page.locator('.explorer-row-topics')).not.toContainText('ideas.')
