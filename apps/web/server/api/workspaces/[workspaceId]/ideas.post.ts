...
import { optionalInitiativeType } from '../../../utils/initiative'
...
export default defineEventHandler(async (event) => {
  ...
  const initiativeType = optionalInitiativeType(body?.initiative_type)
  if (!title || !pitch) {
  ...
  const result = await depositWorkspaceIdea({
    ...
    body: { title, pitch, lang, initiative_type: initiativeType },
  })
  if (result.error) {
