...
- `rg` for `memolabs\.dev|159\.195\.213\.26|/Users/memo|ssh lab|zfbse6gg3ilyvl8194xd6` over the tracked tree: the pilot host survives only in `apps/web/nuxt.config.ts` and `apps/web/server/utils/kollio-api.ts` (the deployment change), and no server address, home directory path or application id remains anywhere.
- `pnpm lint`: clean.
