...
Status: **passing.** Every row names a test that exists and runs green.
...
| Raw Branch material never promoted to a Contribution is never sent | `tests/integration/test_challenge_execution.py::test_raw_branch_material_never_reaches_the_brief` (a Branch title sitting in the Space appears nowhere in the brief, which carries the confirmed Contribution alone) | passing |
...
| A failed run can be tried again and its reason stays readable | `tests/integration/test_challenge_execution.py::test_a_failed_run_keeps_its_reason_and_a_new_one_can_be_opened` (a second run opens on the same Option while the failed row keeps its `FAILED` status and its reason) | passing |
...
| An English run behaves identically | `tests/integration/test_challenge_execution.py::test_an_english_run_asks_the_critic_in_english` (the gateway receives `locale == "en"`) | passing |
...
Result: `13 passed` (`test_challenge_execution.py`), `22 passed` (`test_challenge.py`), 26 passed (`tests/unit/challenge`).
...
None. The three gaps recorded when this change was archived are closed: a Branch title sitting in the Space is asserted absent from the brief, a second challenge opens on an Option after a failed run while the failed row keeps its reason, and the English locale is asserted reaching the gateway.
