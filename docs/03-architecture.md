...
- **Pipeline de raisonnement (#76)** : derrière UNE analyse contrainte (`agent/pipeline.py`), cinq étapes ordonnées — analyste (cas favorable, commodité), challenger (pourquoi ça échoue chez EUX), critique de preuves (fait vs spéculation), company fit (objectifs/contraintes actifs), puis synthétiseur (premium) qui lit le challenger et la critique. `require_synthesis_inputs` refuse une conclusion qui sauterait l'un des deux ; sinon échec non silencieux. Les étapes sont persistées par run (`constraint_analyses.steps`, `analysis_workflows.draft_steps`) pour rejeu et audit. Un brief de décision compact, jamais tout l'historique. La surface membre ne change pas : une seule réponse d'analyse, les coûts sont dépensés là où ça se ressent (cheap prépare, premium challenge et synthétise).
